...
const evenNumbers = mixedNumbers.filter(onlyEven)
...
const postTaxPrices = prices.map(irs);
...
const totalPopulation = populations.reduce(add)
...
const monstersInYourPocket = [
  ...
  {"monster":"Bulbabunny","CP":250},{"monster":"Ponylopse","CP":277},
...
// Do not edit the code above.
// console.log(monstersInYourPocket[0]["CP"])
/*
...
const plusTax = orders.map(function (value) {
  return value.price + (value.price * value.tax);
});
console.log(plusTax)


// const plusTax = orders.reduce((prevVal, curVal) => {
//   let price = curVal.price;
//   let tax = curVal.tax;
//   let total = price + (price * tax);
//   return prevVal + total}, 0);
// console.log(plusTax);
...
const bobOwes = purchases.reduce(function (total, value) {
  if (value.owner === "Bob") {
...
function myFunction (str) {
  let newStr = str.slice(str.length-4, 3);
  return newStr;
}
